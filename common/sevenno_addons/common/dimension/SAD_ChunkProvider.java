...
public class SAD_ChunkProvider implements IChunkProvider
{
	private Random rand;
	...
	public SAD_ChunkProvider(World world, long seed, boolean features)
	{
		this.worldObj = world;
	...
	public void func_147424_a(int chunkX, int chunkZ, Block[] surfaceBlock)
	{
		...
		{
			int l = k * 5;
			int i1 = (k + 1) * rand.nextInt(5); // default : 5 without rand.nextInt()
					...
					for(int l2 = 0; l2 < 8; ++l2)
					{
						...
						for(int i3 = 0; i3 < 4; ++i3)
						{
							...
							for(int k3 = 0; k3 < 4; ++k3)
							{
								if((d15 += d16) > 0.0D)
								{
									surfaceBlock[j3+=short1] = SABlockList.DStone;
								}
								else if(k2 * 8 + l2 < b0)
								{
	...
	public void replaceBlocksForBiome(int x, int z, Block[] block, byte[] arrayOfByte, BiomeGenBase[] biomeList)
	{
		...
		for(int k = 0; k < 16; ++k)
		{
			for(int l = 0; l < 16; ++l)
			{
				DimensionGenBiomeOre biomegenbase = (DimensionGenBiomeOre)biomeList[l + k * 16];
	...
	{
		DimensionGenBiomeOre dimCreature = (DimensionGenBiomeOre)this.worldObj.getBiomeGenForCoords(x, z);
